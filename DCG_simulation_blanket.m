...
% *uses either Anderson linear or Roering-Perron-type nonlinear diffusion.
% *** This simulation has no discrete englacial debris sources, but instead
% a debris blanket of varying thickness defined in the domain set-up.
%
% #########################################################################
...
D0 = 0.5;  % m^2/yr [per 1/1 slope]; a maximum for large thickness
...
thick0 = 0.05;          % m, reference debris thickness for hyperbolic 
                            % decay, equivalent to H*

% concentration and volume adjustments for debris melt-out
deb_conc = 0.0;         % volumetric debris concentration, default = 0
deb = deb_conc*ones(1,nx); % 1-D array for debris_conc as function of x
...
dt = 0.25*(dx^2)/(2*D0); % years. this is currently a fixed time-step
...
% lines below create non-uniform ice surface below debris. No need to
% change unless seeking new debris-blanket geometry
zdeb0 = zinit*ones(1,nx);
zice0 = zdeb0;
zice0thin = zinit-mindebthick; zice0thick = zinit-maxdebthick;
rampstart = ceil((nx-transzonewidth)/2); 
rampend = ceil((nx+transzonewidth)/2);
Xq = X(rampstart:rampend);
zice0(1:rampstart)=zice0thin;
zice0(rampend:end)=zice0thick;
zice0(rampstart:rampend) = ...
    interp1([(rampstart-1)*dx (rampend-1)*dx],[zice0thin zice0thick],Xq); % PROBLEM?!<<<<<<<<<<<<<<<<<

zi = zice0;

% ####### initialize variable arrays ####################
zdeb = zeros(nx,nt); 
zdeb(:,1) = zdeb0; %.*ones(nx,1);
